$(document).ready(function() {
    ...
    $(document).ready(function() {
    // Existing code for toggle change
    ...
    // New code to check initial state on page load
    if ($('.toggle-dark input').is(':checked')) {
        $('body').addClass('theme-dark');
    }
});
...
// Simula la pseudo-elemento ::after dinamicamente
underline.style.position = 'absolute';
underline.style.height = '2px';
underline.style.backgroundColor = 'var(--theme-color, #00ffff)';
...
if (tab.classList.contains('active')) {
    window.addEventListener('load', () => moveUnderline(tab));
}
});
